...
data class Thang(override val name: String, val parent: String? = null) : Named

class NamedString(val s: CharSequence) : CharSequence by s, Named {
  override val name: String = s.toString()
  override fun toString(): String = s.toString()
}

val CharSequence.named: NamedString
  get() = NamedString(this)

suspend fun test2(args: Array<String>) {
  println("test2(): args: ${args.contentToString()}")
  val envProperties = propertyResolver("_", getter = ::getEnv).cached().default { name, parent ->
    //println("GETTING DEFAULT VALUE FOR [$name:$parent]")
    "$name:$parent"
  }
  args.forEach { key ->
    println("envProperties[$key] = ${envProperties.defaultValue(key)}")
  }
